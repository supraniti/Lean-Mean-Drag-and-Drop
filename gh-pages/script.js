...
var scrollControl = {
    ...
    },//visual element height (without scroll bar)
    get veh2() {
        return (this.el === document.documentElement) ? window.innerHeight : this.el.offsetHeight;
    },//visual element height (with scroll bar)
    get rew() {
        return this.el.scrollWidth;
    },//real element width
    get vew1() {
        return this.el.clientWidth;
    },//visual element width (without scroll bar)
    get vew2() {
        return (this.el === document.documentElement) ? window.innerWidth : this.el.offsetWidth;
    },//visual element width (with scroll bar)
    get cspy() {
        return (this.el === document.documentElement) ? window.pageYOffset : this.el.scrollTop;
    },//current scroll position on y axis
    get cspx() {
        return (this.el === document.documentElement) ? window.pageXOffset : this.el.scrollLeft;
    },//current scroll position on x axis
    get cmpy() {
        return (this.el === document.documentElement) ? this.event.clientY : this.getOffsetY();
    },//current mouse position y
    get cmpx() {
        return (this.el === document.documentElement) ? this.event.clientX : this.getOffsetX();
    },//current mouse position x
    get asm() {
        return this.scrollMargin / window.devicePixelRatio;
    },//adjust scroll margin to browser zoom
    get mspy() {
        return this.reh - this.veh1;
    },//maximum scroll position on y axis
    get mspx() {
        return this.rew - this.vew1;
